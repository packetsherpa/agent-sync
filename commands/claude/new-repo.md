---
description: Scaffold a new repo from a copier template (AGENTS.md standard)
argument-hint: [repo-name] [one-line description]
allowed-tools: Bash(uvx copier:*), Bash(git init:*), Bash(git add:*), Bash(git commit:*), Bash(gh repo create:*)
---
...
Steps:
1. Pick a destination directory — default `./<repo-name>` unless I say otherwise.
...
5. Confirm the result: `AGENTS.md` exists, `CLAUDE.md` and `GEMINI.md` are symlinks to it, and `.claude/memory/MEMORY.md` is present.
